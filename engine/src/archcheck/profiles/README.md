...
| `entry_functions` | `rule` `function` `kind`（目前只有 `main`）`superloop` | 框架代为调用的入口（Arduino 的 `setup` / `loop`）。名字太普通，只在工程自己没有 `main` / `app_main` 时才认。`superloop: true` 表示整个函数体是大循环的一轮，运行模式按函数体里的阻塞点推。出到 `entries[]`，带 `rule` 与 `superloop`。/ Entries the framework calls on the project's behalf; only applied when the project defines no `main`. `superloop: true`: the whole body is one pass of the main loop. |
