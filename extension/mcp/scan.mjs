...
export function scanProject({ folder, stateDir, outDir, sourceScanOnly = false }) {
  ...
  if (!engine) {
    return {
      ...
      fix: ["Install the ArchX VS Code extension (it bundles the engine and registers it on activation)", "or set ARCHX_ENGINE to an archcheck executable / engine source directory", "or: pip install <path-to-repo>/engine"],
    };
  }
